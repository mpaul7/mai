...
from .common import DefaultAbstractModel, PositionalEncoding, load_model
...
class ThreeFeatureTypeModelFactoryMixin:
    def _create_keras_model(self, preprocessor_flow, preprocessor_pkt, params, dataset):
        ...
        if 'mlp' in params.structure:
            ...
            for i in range(params.num_dense):
                ...
                flow_x = layers.Dense(units=params.units_list[i])(flow_x)
                # flow_x = layers.BatchNormalization()(flow_x)
                flow_x = layers.LeakyReLU()(flow_x)

                # Recurrent connection around these two dense layers
                recurrent_x = flow_x
        ...
        if 'lstm' in params.structure:
            ...
            #                                         input_shape=(params.sequence_length, len(params.packet_feature_name[:2])),
            ...
            # when num_lstm == 0, create a Attention layer instead
            # if params.num_lstm == 0:
            #     attn_x = pktseq_x
                
            #     pktseq_x = PositionalEncoding()(pktseq_x)

            #     pktseq_x = layers.MultiHeadAttention(num_heads=5,
            #                                          key_dim=pktseq_x.shape[-1],
            #                                          dropout=0.2)(pktseq_x, pktseq_x)
            #     pktseq_x = layers.LayerNormalization(epsilon=1e-6)(pktseq_x + attn_x)
            ...
            #     pktseq_x = layers.Flatten()(pktseq_x)
        ...
        if 'cnn' in params.structure:
            ...
            ## Initial cnn layers
            raw_x = layers.Conv1D(filters=params.num_filter,
                                  kernel_size=11,
                                  strides=1,
                                  padding='same',
                                  activation='relu',
            ...
            raw_x = layers.AveragePooling1D(pool_size=4, strides=4, padding='same')(raw_x)

            ## Additional cnn layers
            # n_filters = params.num_filter/4
            for i in range(params.num_cnn):
                n_filters = int(params.num_filter * (0.5 * i + 1))
                raw_x = layers.Conv1D(filters=n_filters,
                                      kernel_size=5,
                                      strides=1,
                                      padding='same',
                                      activation='relu',
                ...
                raw_x = layers.AveragePooling1D(pool_size=2, strides=2, padding='same')(raw_x)

            attn_x = raw_x

            raw_x = PositionalEncoding()(raw_x)
            
            raw_x = layers.MultiHeadAttention(num_heads=3, key_dim=params.num_filter)(raw_x, raw_x)
            # raw_x = layers.MultiHeadAttention(num_heads=6, key_dim=params.num_filter, dropout=0.1)(raw_x, raw_x)
            raw_x = layers.LayerNormalization(epsilon=1e-6)(raw_x + attn_x)
            ...
            if params.num_cnn_dense == 0:
                ...
                for i in range(params.num_cnn_dense):
                    ...
                    raw_x = layers.Dense(units=params.cnn_dense_units_list[i])(raw_x)
        ...
        return models.Model(inputs=input_branches.get('inputs'), outputs=outputs)
    ...
    def _create_preprocess_layer_flow(self, features, dataset):
        ...
        dataset = dataset.take(2000).map(featuremap)
    ...
    def _create_preprocess_layer_pkt(self, features, dataset):
        ...
        data = np.stack([np.stack(list(x.values()), axis=1) for x in dataset], axis=0)
...
class ThreeFeatureTypeDatasetFactoryMixin:
    ...
    def _create_dataset(self, datafile, params, training=False, filter_by_label=False):
        ...
        columns = features + [params.label_column]

        # read and immediately shuffle dataset
        df = pd.read_orc(datafile, columns=columns).sample(frac=1)
        
        # retrieve sequence length from dataset
        params.sequence_length = df.loc[0, params.packet_feature_name[0]].shape[0]
        ...
        # Include only used features
        X = df.loc[:, features]

        # Convert to one-hot representation
        y = pd.get_dummies(y)

        print(f"The dataset size is {df.shape[0]}")

        # Split dataset into train/valid chunks
        if filter_by_label:
            ...
            if training:
                return self._create_balanced_tf_dataset(X, params, y), validation_dataset
            else:
                return self._create_tf_dataset(X, params, y), validation_dataset
        else:
    ...
    def _create_balanced_tf_dataset(self, X, params, y):
        df = pd.DataFrame({params.label_column: y.idxmax(axis=1)})

        partials = []
        for _, group in df.groupby(params.label_column):
            partials.append(self._create_tf_dataset(X.loc[group.index], params, y.loc[group.index]).repeat())

...
from dl_models_enta import DLModels, ModelBuilder, ModelBuilderTransferLearning
...
def mai_ml_train(train_data_file, config_file):
    ...
    params['model_plot'] = f"{params['project_home']}/models/model_jan13_test_enta/{model_name}_{current_datetime}.png"
    ...
    _model_arch  = model_builder.build_model()
     
    # """Save model architecture in Json config file"""
    model_json = _model_arch.to_json()
    with open(f"{params['project_home']}/models/model_jan13_test_enta/{MODEL_JSON}", "w") as f:
        json.dump(model_json, f)
        
    with open(f"{params['project_home']}/models/model_jan13_test_enta/{MODEL_JSON}") as f:
    ...
    model.save(f"{params['project_home']}/models/model_jan13_test_enta/{MODEL_H5}", save_format='h5')
...
def test_model_cmd(trained_model_file, config_file, test_data_file):
    ...
    if trained_model_file is not None:
        ...
        output_file = os.path.join( f'{params["project_home"]}/results/results_jan13/cross_dataset_v2/', tail.replace('.h5', '.csv'))
